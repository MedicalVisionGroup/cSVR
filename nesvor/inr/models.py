...
class NeSVoR(nn.Module):
    ...
    def build_network(self, bounding_box) -> None:
        ...
        # print("DEBUG logit_coef:", self.logit_coef.detach().cpu().numpy())
        # print("DEBUG softmax(logit_coef):", F.softmax(self.logit_coef, 0).detach().cpu().numpy())    
    ...
    def forward(
      ...
      #  print("3D psf sigma")
        # transform points
        t = self.axisangle[slice_idx][:, None] # slice wise, is this spherical gaussian?
       ...
       # pdb.set_trace()
        ...
        if not self.args.no_slice_scale:
            ...
            ## CORRECTED LINES: Use Vectorized Masking on the GPU
            
            # 1. Initialize c as a tensor of 1.0s (scalar float)
            
#            c = torch.ones_like(v, dtype=torch.float32, device=slice_idx.device).mean(dim=-1)
            
            # 2. Define boolean masks for the three index ranges
            
            # Mask 1: indices 16 to 20
 #           mask_1 = (slice_idx >= 16) & (slice_idx <= 20) 
            # Mask 2: indices 20 to 27
  #          mask_2 = (slice_idx >= 20) & (slice_idx <= 27)
            # Mask 3: indices 45 to 51
   #         mask_3 = (slice_idx >= 45) & (slice_idx <= 51)
            
            # Combine all three masks
    #        combined_mask = mask_1 | mask_2 | mask_3
            
            # Apply the desired value (1.8) using torch.where
            # torch.where(condition, value_if_true, value_if_false)
     #       c = torch.where(combined_mask, torch.tensor(1.8, device=c.device), c)
