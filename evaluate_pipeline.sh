...
#rot="${10}"  # specify the commit hash you want to run
gt_run_name="${10}"
rot="${11:-0}"
translation="${12:-0}"
noise="${13:-0}"
...
mlp_path="/data/vision/polina/users/mfirenze/cSVR/checkpoints/${mlp_path}"
...
echo $translation
echo $noise
python utils_evaluation_crop.py --folder_save $folder_save --model_folder $ckpt_path --mlp_folder $mlp_path --img_num $img_num --gt "$gt" --hres "$hres" --save_gt $save_gt --clin "$clin" --rot "$rot" --translation "$translation" --noise "$noise"
...
if [[ ( "$recon_method" == "only_svr" && "$gt" == "False" ) || ( "$gt" == "True" && "$clin" == "False" ) ]]; then
...
if [[ "$gt" == "False" && "$clin" == "True" ]]; then
    cd /data/vision/polina/users/mfirenze/splatting_exp/inference_recons
    echo "run_name=$run_name, output_sim=$output_sim, json_path=$json_path, img_num=$img_num"
    python /data/vision/polina/users/mfirenze/svr_my_train_2024/get_TRE_from_file_outputs.py \
        --mask_folder /data/vision/polina/users/mfirenze/splatting_exp/outputs_gen_vols/slices_$run_name \
        --folder_est /data/vision/polina/users/mfirenze/splatting_exp/inference_recons/$output_sim \
        --json_path /data/vision/polina/users/mfirenze/svr_my_train_2024/evaluate_metrics/$json_path \
...
if [[ "$gt" == "False" && "$clin" == "True" && "False" == "True" ]]; then
    cd /data/vision/polina/users/mfirenze/splatting_exp/inference_recons
    #reference_file="svr_val15_gt.nii.gz"
    reference_file="svr_${run_name}_gt.nii.gz"
    reg_file="reg_${output_file}"
