...
class PointDataset(object):
    ...
    def mask(self) -> Volume:
        with torch.no_grad():
            resolution_min = self.resolution.min() #1
            resolution_max = self.resolution.max() #4
            xyz = self.xyz_transformed #all coordinates covered
            xyz_min = xyz.amin(0) - resolution_max * 10 # 10 voxel boundary
            xyz_max = xyz.amax(0) + resolution_max * 10 # 10 voxel boundary
            shape_xyz = ((xyz_max - xyz_min) / resolution_min).ceil().long()
            shape = (int(shape_xyz[2]), int(shape_xyz[1]), int(shape_xyz[0]))
            kji = ((xyz - xyz_min) / resolution_min).round().long() # center at zero
         #   print("POINT MASK")
            ##pdb.set_trace()
            mask = torch.bincount( #calculates frequency of each 3D coordinate, for example 116,118,248 appears 3 times
                kji[..., 0]
                + shape[2] * kji[..., 1]
         ...
         #   print("IN MASK KJI")
            ...
            # sum of non zero values / total non zero pixels
            # threshold is average coordinate appearence 
            mask_threshold *= mask.sum() / (mask > 0).sum() # 0.5167 multi stack, 0.25 for stack
         #   ("MASK THRESHOLD OG")
         #   print(mask_threshold)
          #  mask_threshold = 0.11
         #   mask_threshold = 0 #0.5  # for thickness 4 0.33, works for thickness 1
         #  mask_threshold = 0.3 #0.5  # for thickness 4 0.33
         #   print("MASK THRESHOLD NEW!")
         #   print(mask_threshold)
            ...
            if(gauss_mask):
                ...
                mask = mask > 0
           #     print("no mask gauss")
            # torch.all(mask == False)  -> True when 1 stack, tensor(False, device='cuda:0') -> False for mutli stack
