...
mkdir -p "${DATA_DIR}/${FILE_NAME}/svort_files"

nesvor svr --input-stacks $stack0 $stack1 $stack2 \
    --stack-masks $mask0 $mask1 $mask2 \
    --output-slices "${DATA_DIR}/${FILE_NAME}/svort_files/${FILE_NAME}_sim_slices_${SUFFIX}" \
    --simulated-slices "${DATA_DIR}/${FILE_NAME}/svort_files/${FILE_NAME}_out_slices_${SUFFIX}" \
    --output-volume "${DATA_DIR}/${FILE_NAME}/svort_files/${FILE_NAME}_${SUFFIX}.nii.gz" \
    --no-global-exclusion --n-iter 5 --n-iter-rec 3 \

    
python /data/vision/polina/users/mfirenze/cSVR/get_TRE_from_file_outputs.py \
    --og_slices "${DATA_DIR}/${FILE_NAME}/svort_files/${FILE_NAME}_out_slices_${SUFFIX}" \
    --folder_est "${DATA_DIR}/${FILE_NAME}/svort_files/${FILE_NAME}_sim_slices_${SUFFIX}"  \
    --json_path "${SCRIPT_DIR}/evaluate_metrics/${SUFFIX}${IDX}.json" \
    --img_num 0 \
    --clin "True"


#export SUFFIX="svort_norm_v2_"
#jobid=$(envsubst '$SUFFIX' < job_eval_clin_svort.sh | sbatch | awk '{print $4}')
