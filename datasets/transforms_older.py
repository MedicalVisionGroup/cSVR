...
class RandAffine3dSlice:
  ...
  #  def __init__(self, spacing=1, translations=0.1, rotations=0, bulk_translations=0, bulk_rotations=0, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3):
         
         # SWITCHED TO TRANS TORCH
      #  trans_torch = torch.tensor([0.1,0,0])
       # trans_torch = torch.tensor([0.07,0,0])
       # trans_torch = torch.tensor([0,0,0])
       # trans_torch = torch.tensor([0,0,0.1])
      #  rotations = 0 
       # bulk_translations = 0 
       # bulk_rotations= 0 
      ...
      #  print("subsample: ")
      #  print(subsample)
      #  print("in initialization randaffine")
...
class GenerateMotionTrajectory:
    # Firs tline is original 
    # changed bulk translations to 0 
    #def __init__(self, spacing=1, translations=0, rotations=20, bulk_translations=0, bulk_rotations=180, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3):
    # original one for training
   # def __init__(self, spacing=1, translations=0.10, rotations=20, bulk_translations=0, bulk_rotations=180, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3, flow_final=False):

    # one for second training less motion
    def __init__(self, spacing=1, translations=0.05, rotations=10, bulk_translations=0, bulk_rotations=10, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3, flow_final=False):
    # very small mot
  #  def __init__(self, spacing=1, translations=0.02, rotations=5, bulk_translations=0, bulk_rotations=5, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3, flow_final=False):

        print("IN NEW RAND SLICE AFFINE OCT 15")
        if(translations==0.02):
            print("very small mot")
        if(translations==0.05):
            print(" small mot")
      ...
      #  trans_torch = torch.tensor([0,0,0])
        ...
        self.X = X
    def turn_stacks_planar(self, og):
        ss = og.shape[1]//3
        new_vol = og.clone()
        stack0 = og[:,0:ss,:,:]
        stack1 = og[:,ss:ss*2,:,:]
        ...
        stack2_n = torch.rot90(stack2, k=1, dims=(1, 2)) # for slice1
        stack2_n = torch.rot90(stack2_n, k=-1, dims=(1, 3)) #for slice2
       # print(stack2_n.shape)
    ...
    def flow_turn(self, flow_og): #input is flow 768x256x256
        ss = flow_og.shape[1]//3 #1x3x768x256x256
    ...
    def __call__(self, img1, seg1):
        ...
        numstacks = 3 #torch.randint(1, len(self.slice) + 1, [1]).item()
        img1 = (img1.clamp(min=0.1) - 0.1) * (1 / 0.9)
        seg1 = ((img1 > 0) | (seg1 > 0)).float()
      ...
      #  pdb.set_trace()
       ...
       # print("input planar")
        ...
        img0 = self.turn_stacks_planar(img0)
      #  print(flow.shape)
        flow = self.flow_turn(flow)

        if (self.flow_final):
           #  flow_tot = flow_ot + item[1][None].flip(1)[:,1:,:,:,:]
             flow_ot = flow_to_orthogonal(flow[None])
             new_flow = torch.zeros_like(flow).to(flow.device)
             #pdb.set_trace()
             new_flow[1:4] = flow_ot+ flow[None].flip(1)[:,1:,:,:,:]
             new_flow[0] = flow.flip(1)[None][0,0,:,:,:]

           #  new_flow[0:3] = flow_ot+ flow[0:3,:,:,:]
            # new_flow[3] = flow[3,:,:,:]
             flow = new_flow
             
            # flow[0:3] = flow[0:3]+ flow_ot
             #flow = flow_ot + flow[None].flip(1)

       # print(flow.shape)
        # try to reduce precisoin
      #  img0 = img0.half()  # Convert img0 to float16
      #  flow = flow.half()  # Convert flow to float16
      #  print("did half")

        return img0, flow
    

class GenerateMotionTrajectoryOld:
    # Firs tline is original 
    # changed bulk translations to 0 
    #def __init__(self, spacing=1, translations=0, rotations=20, bulk_translations=0, bulk_rotations=180, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3):
    # original one for training
    ...
    # one for second training less motion
   # def __init__(self, spacing=1, translations=0.05, rotations=10, bulk_translations=0, bulk_rotations=10, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3):
...
class GenerateMotionTrajectoryOLD2:
    # Firs tline is original 
    # changed bulk translations to 0 
    #def __init__(self, spacing=1, translations=0, rotations=20, bulk_translations=0, bulk_rotations=180, zooms=0, subsample=1, slice=1, nodes=(8,16), shots=2, augment=True, noise=False, X=3):
    # original one for training
